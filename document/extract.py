...
_PURPOSE_RE = re.compile(
    ...
    r"|to familiarize|designed to|intends? to|in response to)", re.IGNORECASE)
...
def extract_summary(text: str, doc_type: str, location: str, date: str) -> str:
    ...
    for i, l in enumerate(raw):
        if _PURPOSE_HDR.match(l):
            ...
            if s:
                return s
            break
    # Drop letterhead lines, then work with the remaining prose
    lines = [l for l in raw if not _is_letterhead_line(l)]
    clean = re.sub(r"\n{2,}", " ", "\n".join(lines)).strip()
    ...
    for s in sentences:
        ...
        if m and not _is_letterhead_sentence(s):
            prefix = s[:m.start()]
            if prefix.strip() and (re.search(r"\d", prefix)
                                   or _is_letterhead_sentence(prefix)):
                return s[m.start():].strip()[:300]
            return s[:300]
...
def extract_fields(seg: dict, nlp) -> dict:
    ...
    location = extract_location(text, ner)
    row = {
        "Report_ID": seg["report_id"],
        "Incident_Type": seg["doc_type"],
        "Date": date,
        "Location": location,
        "Officer": _normalize_name(extract_officer(text, ner)),
